...
let return: 'a => t('a) = (x, ()) => Cons(x, empty);

// Converts a sequence into a list.
let rec to_list: t('a) => list('a) =
  seq => {
    switch (seq()) {
    | Nil => []
...
// applies when the result is traversed.
let rec filter_map: ('a => option('b), t('a)) => t('b) =
  (f, seq) => {
    switch (seq()) {
    | Nil => empty
    | Cons(x, xs) =>
...
// result is traversed.
let rec flat_map: ('a => t('b), t('a)) => t('b) =
  (f, seq) => {
    switch (seq()) {
    | Nil => empty
...
let rec fold_left =
        (f: ('acc, 'ele) => 'acc, accumulator: 'acc, seq: t('ele)): 'acc => {
  switch (seq()) {
  | Nil => accumulator
